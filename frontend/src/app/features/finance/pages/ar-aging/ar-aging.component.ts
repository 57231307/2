  ...
  template: `
    <div class="p-6">
      ...
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">应收款账龄分析</h1>
      </div>
              ...
              <table class="w-full">
                <thead class="bg-red-50">
                  <tr>
                    ...
                    <th class="px-4 py-2 text-left text-sm font-medium text-red-700">客户名称</th>
                    <th class="px-4 py-2 text-right text-sm font-medium text-red-700">应收金额</th>
                    <th class="px-4 py-2 text-right text-sm font-medium text-red-700">待收金额</th>
                ...
                <tbody class="divide-y divide-red-100">
                  @for (alert of overdueAlerts(); track alert.id) {
                    <tr class="hover:bg-red-50">
                      ...
                      <td class="px-4 py-2 text-sm text-gray-800">{{ alert.customerName }}</td>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="card">
          <div class="p-4 border-b">
            <h2 class="text-lg font-bold text-gray-800">客户分布</h2>
          </div>
          <div class="p-4">
            @if (agingReport()?.customers?.length) {
              <table class="w-full">
                <thead class="bg-gray-50">
                  <tr>
                    <th class="px-4 py-2 text-left text-sm font-medium text-gray-600">客户名称</th>
                    <th class="px-4 py-2 text-right text-sm font-medium text-gray-600">金额</th>
                    <th class="px-4 py-2 text-right text-sm font-medium text-gray-600">占比</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                  @for (customer of agingReport()?.customers || []; track customer.customerName) {
                    <tr class="hover:bg-gray-50">
                      <td class="px-4 py-2 text-sm text-gray-800">{{ customer.customerName }}</td>
                      <td class="px-4 py-2 text-sm text-right font-medium">¥{{ customer.amount | number: '1.2-2' }}</td>
                      <td class="px-4 py-2 text-sm text-right text-gray-600">{{ customer.percentage }}%</td>
                    </tr>
                  }
                </tbody>
              </table>
            } @else {
              <div class="text-center text-gray-500 py-8">暂无客户分布数据</div>
            }
          </div>
        </div>
      </div>
...
export class ArAgingComponent implements OnInit {
  ...
  loadAgingReport(): void {
    this.financeService.getArAgingReport().subscribe({
      next: (result: any) => {
        // 将后端返回的中文键名转换为英文
  ...
  loadOverdueAlerts(): void {
    this.financeService.getArOverdueAlerts().subscribe({
      next: (result: any) => {
